...
# Original Author: Jin Lee (leepc12@gmail.com)
# mods: N Javed
...
workflow atac {
	...
	Boolean true_rep_only = true 	# disable all analyses involving pseudo replicates (including overlap/idr)
	...
	Boolean enable_jsd = false 		# enable JSD plot generation (deeptools fingerprint)
	...
	Int align_cpu = 8
	Int align_mem_mb = 24000
	Int align_time_hr = 48
	String align_disks = 'local-disk 300 HDD'

	Int filter_cpu = 2
	Int filter_mem_mb = 10000
	Int filter_time_hr = 12
	String filter_disks = 'local-disk 300 HDD'
	...
	output {
        ...
        Array[File?] frac_mito_qc = frac_mito.frac_mito_qc
        Array[File?] gc_log = gc_bias.gc_log
        Array[File?] pval_bw = macs2_signal_track.pval_bw
        Array[File?] fc_bw = macs2_signal_track.fc_bw
        Array[File?] bam = filter.nodup_bam
        Array[File?] bed = call_peak.bfilt_peak
...
task align {
	...
	output {
	 ...
	 runtime {
        docker : "quay.io/encode-dcc/atac-seq-pipeline:v1.5.4"
        preemptible : 2
        cpu : cpu
		memory : '${mem_mb} MB'
		time : time_hr
