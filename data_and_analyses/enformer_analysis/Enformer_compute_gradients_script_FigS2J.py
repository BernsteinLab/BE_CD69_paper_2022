...
### define enformer model path
model = Enformer(model_path)
fasta_extractor = FastaStringExtractor(fasta_file)
#### where to center the window
chrom, start, stop = sys.argv[2].split(',')

### where to mask out (i.e. calculate gradients relative to)
mask_indices = [int(k) for k in sys.argv[3].split(',')]
## which cell-type
target_index = sys.argv[4]
## target output name
out_name = sys.argv[5]
## give peak files to subset the areas where you extract gradients, significantly reduces modisco run time
peaks = sys.argv[6]
